defmodule Theme01Web.TeamControllerTest do
  use Theme01Web.ConnCase

  alias Theme01.Teams.Team
  alias Theme01.Teams
  alias Theme01.Users

  @create_attrs %{
    name: "some name"
  }
  @update_attrs %{
    name: "some updated name"
  }
  @invalid_attrs %{name: nil}

  setup %{conn: conn} do
    {:ok, conn: put_req_header(conn, "accept", "application/json")}
  end

  setup [:create_user_and_assign_valid_jwt]

  describe "index" do
    test "lists all teams", %{conn: conn} do
      {:ok, user} =
        Users.create_user(%{
          email: "test@test.com",
          username: "test",
          role: "Manager",
          password_hash: "password"
        })

      conn = get(conn, Routes.team_path(conn, :index, user.id))

      assert json_response(conn, 200)["data"] == []
    end
  end

  describe "create team" do
    test "renders team when data is valid", %{conn: conn} do
      {:ok, user} =
        Users.create_user(%{
          email: "test@test.com",
          username: "test",
          role: "Manager",
          password_hash: "password"
        })

      conn = post(conn, Routes.team_path(conn, :create, user.id), team: @create_attrs)
      assert %{"id" => id} = json_response(conn, 201)["data"]

      conn = get(conn, Routes.team_path(conn, :show, user.id, id))

      assert %{
               "id" => ^id,
               "name" => "some name"
             } = json_response(conn, 200)["data"]
    end

    test "renders errors when data is invalid", %{conn: conn} do
      {:ok, user} =
        Users.create_user(%{
          email: "test@test.com",
          username: "test",
          role: "Manager",
          password_hash: "password"
        })

      conn = post(conn, Routes.team_path(conn, :create, user.id), team: @invalid_attrs)
      assert json_response(conn, 422)["errors"] != %{}
    end
  end

  describe "update team" do
    setup [:create_team]

    test "renders team when data is valid", %{conn: conn, team: %Team{id: id} = team} do
      conn = put(conn, Routes.team_path(conn, :update, team), team: @update_attrs)

      assert %{"id" => ^id} = json_response(conn, 200)["data"]

      conn = get(conn, Routes.team_path(conn, :show, team.manager_id, id))

      assert %{
               "id" => ^id,
               "name" => "some updated name"
             } = json_response(conn, 200)["data"]
    end

    test "renders errors when data is invalid", %{conn: conn, team: team} do
      conn = put(conn, Routes.team_path(conn, :update, team), team: @invalid_attrs)

      assert json_response(conn, 422)["errors"] != %{}
    end
  end

  describe "delete team" do
    setup [:create_team]

    test "deletes chosen team", %{conn: conn, team: team} do
      conn = delete(conn, Routes.team_path(conn, :delete, team))
      assert response(conn, 204)

      assert_error_sent(404, fn ->
        get(conn, Routes.team_path(conn, :show, team.manager_id, team))
      end)
    end
  end

  defp create_team(_) do
    team = fixture(:team)
    %{team: team}
  end

  def fixture_user(:user) do
    valid_user_attrs = %{
      username: "Some user",
      email: "someuse@mail.com",
      role: "Manager",
      password_hash: "password"
    }

    {:ok, user} = Users.create_user(valid_user_attrs)
    user
  end

  def fixture(:team) do
    user = fixture_user(:user)
    team_params = %{manager_id: user.id, name: "Team A"}

    {:ok, team} = Teams.create_team(team_params)

    team
  end
end
